...
def test_launchpads():
    launchpads_data=''
    launchpads_result=keyOrder(alphaOrder(readJSONFile('launchpads/all.json')),'id')
    try:
        launchpads_data = keyOrder(alphaOrder(spacexpython.launchpads.launchpads('',1)),'id')
...
def test_one_landingpad():
    ...
    try:
        one_data = alphaOrder(spacexpython.launchpads.one('ksc_lc_39a','',1))
    except spacexpython.utils.SpaceXReadTimeOut:
        pytest.xfail("Space/X API Read Timed Out")
