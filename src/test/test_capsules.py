...
def test_capsules():
    capsules_data=''
    capsules_result=keyOrder(alphaOrder(readJSONFile('capsules/capsules.json')),'core_serial')
    try:
        capsules_data = keyOrder(alphaOrder(spacexpython.capsules.capsules('',1)),'core_serial')
...
def test_upcoming():
    upcoming_data=''
    upcoming_result=keyOrder(alphaOrder(readJSONFile('capsules/upcoming.json')),'core_serial')
    try:
        upcoming_data = keyOrder(alphaOrder(spacexpython.capsules.upcoming()),'core_serial')
    except spacexpython.utils.SpaceXReadTimeOut:
        pytest.xfail("Space/X API Read Timed Out")
...
def test_past():
    past_data=''
    past_result=keyOrder(alphaOrder(readJSONFile('capsules/past.json')),'core_serial')

    try:
        past_data = keyOrder(alphaOrder(spacexpython.capsules.past()),'core_serial')
    except spacexpython.utils.SpaceXReadTimeOut:
        pytest.xfail("Space/X API Read Timed Out")
